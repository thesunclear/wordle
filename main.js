...
async function suggestNext() {
	...
	try {
		...
		if (hard) {
			...
			status.textContent = `Evaluating top ${total} words (from the top-ranked candidates and external pool)...`;
		}
		...
		// 4) Render results (even if stopped in the middle, rows up to that point are shown)
