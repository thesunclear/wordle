...
*/
function EstepsOneLookaheadLocalDeeper(S, g) {
  const n = S.length;
...
/* Deepest E[steps] with an extra lookahead level:
   - Level 1: g (the candidate we are evaluating now)
   - Level 2: choose best g2 inside each bucket after g
   - Level 3: choose best g3 inside each bucket after g2
   - Level 4: choose best g4 inside each bucket after g3
...
const HIDDEN_DEEPER_LIMIT = 200;

function EstepsOneLookaheadLocalDeep(S, g) {
	const n = S.length;
	If (n > HIDDEN_DEEPER_LIMIT) {
		return EstepsOneLookaheadLocalDeeper(S, g); 
	} else {
		return EstepsOneLookaheadLocalDeepest(S, g);
	}
}
...
/* Deeper E[steps] with an extra lookahead level:
   ...
   - The “leaf” buckets add a small extra cost depending on size.
*/
function EstepsOneLookaheadDeeper(S, g) {
  const n = S.length;
  const parts1 = partitionByPattern(S, g);
  let est = 0;
...
/* Deeper E[steps] with an extra lookahead level:
   ...
   - The “leaf” buckets add a small extra cost depending on size.
*/
function EstepsOneLookaheadDeepest(S, g) {
  const n = S.length;
  const parts1 = partitionByPattern(S, g);
  let est = 0;
...
function EstepsOneLookaheadDeep(S, g) {
	const n = S.length;
	If (n > HIDDEN_DEEPER_LIMIT) {
		return EstepsOneLookaheadDeeper(S, g); 
	} else {
		return EstepsOneLookaheadDeepest(S, g);
	}
}
				...
				for (const g2 of subset){
					...
					for (const [pat2, subset2] of part2.entries()){
						...
						if (pat2 === '22222') {
							...
							// const k = subset2.length;
							const expLeaf = (m2===1) ? 1 : 2; // const expLeaf = leafExtraGuesses(k); // 
							e2 += p2 * (1 + expLeaf);
						}
					}
					if (e2 < best) best = e2;
