		...
		d.onclick = () => {
			...
			if (kind === 'gInc') {
				...
				if (!wasIncluded && isNowIncluded) {
					...
					if (pos.exclude.delete(letter)) {
						toast(`Yellow (${letter.toUpperCase()}) cleared for position ${state.activePos+1}`);
					}
					if (state.globalExclude.delete(letter)) {
						toast(`Global Exclude (${letter.toUpperCase()}) removed (conflicted Position Include)`);
					}
				}
			} else if(kind === 'pExc') {
				...
				if (slot.include.size === 1 && slot.include.has(letter)) {
					...
					slot.include.clear();
					// If this letter is no longer used in ANY position
					let stillUsed = false;
					for (let i = 0; i < 5; i++) {
					...
					if (!stillUsed) {
						// No green/yellow constraint remains → remove from globalInclude
						state.globalInclude.delete(letter);
						// Do NOT add to globalExclude here; gray keys are the only
						// thing that should mark a letter as "not in the word".
					}
				} else {
				...
				if (state.globalExclude.has(letter)) {
					...
					state.globalInclude.delete(letter);
					for (let i = 0; i < 5; i++) {
						state.pos[i].include.delete(letter);
						state.pos[i].exclude.delete(letter);
					}
				}
			}
