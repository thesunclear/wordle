...
const MAX_TOASTS = 5;  // Maximum number of items to display simultaneously
 ...
 * options: { duration?: number }  // ms
 */
function showToast(message, type = "info", options = {}){
  const dur = Math.max(1200, options.duration ?? 2400);
  const container = getToastContainer();

  // Excess Cleanup: If the current count reaches the upper limit, remove the oldest (top) one first.
  while (container.children.length >= MAX_TOASTS) {
    // Since it is column-oriented, the first one is the oldest
  ...
  // Since it is a column, use appendChild to stack it at the bottom.
  ...
  const to = setTimeout(hide, dur);

  // Make it so that the toast can be closed by clicking on it (the container has pointer-events:none)
		...
		d.onclick = () => {
			...
			if (kind === 'gInc') {
				...
				toggle(state.globalInclude, state.globalExclude, letter);
					if (hadGray) {
						showToast(`"${letter.toUpperCase()}" was removed from Global Exclude.`, "success");
					}
			} else if (kind === 'gExc') {
				// Global Exclude:
				...
				// Pre-toggle snapshot
				const wasIncluded = pos.include.has(letter);        // Was this letter already included at this position?
				const hadYellow   = pos.exclude.has(letter);        // same-position Yellow before toggle?
				const hadGray     = state.globalExclude.has(letter); // global Gray before toggle?
				// Toggle include <-> exclude for this position
				toggle(pos.include, pos.exclude, letter);
				//    If it just became "included" now, update global sets:
				//    - remove from globalExclude
				const isNowIncluded = pos.include.has(letter);
				if (!wasIncluded && isNowIncluded) {
					if (hadYellow && hadGray) {
						state.globalExclude.delete(letter);
						showToast(`"${letter.toUpperCase()}" was removed from both Global Exclude and Position Exclude (Position ${state.activePos + 1}).`, "success");
					} else if (hadGray && !hadYellow) {
						state.globalExclude.delete(letter);
						showToast(`"${letter.toUpperCase()}" was removed from Global Exclude.`);
					} else if (!hadGray && hadYellow) {
						showToast(`"${letter.toUpperCase()}" was removed from Position Exclude (Position ${state.activePos + 1}).`, "success");
					}
				}
				/* if (!wasIncluded && isNowIncluded) {
					const msgs = [];
				...
				if (slot.include.size === 1 && slot.include.has(letter)) {
					...
					if (slot.exclude.delete(letter)) {
						showToast(`"${letter.toUpperCase()}" was removed from Yellow Letter (Position ${state.activePos + 1}).`, "success");
					}
					if (state.globalExclude.delete(letter)) {
						showToast(`"${letter.toUpperCase()}" was removed from Gray Letter.`, "success");
					}
				}
				...
				if (slot.exclude.has(letter)) {
					...
					if (wasIncluded) {
						...
						showToast(`"${letter.toUpperCase()}" was removed from Green Letter (Position ${state.activePos + 1}).`, "warn");
					}
					if (hadGray) {
						state.globalExclude.delete(letter);
						showToast(`"${letter.toUpperCase()}" was removed from Gray Letter.`, "warn");
					}
					// Letter exists somewhere, so keep it out of globalExclude
					state.globalInclude.add(letter);
				}
			} else if (kind === 'easyGray') {
				// Gray: letter is nowhere in the word
				if (state.globalExclude.has(letter)) {
					state.globalExclude.delete(letter);
				} else {
					state.globalExclude.add(letter);
					state.globalInclude.delete(letter);
					let cleared = false;
					for (let i = 0; i < 5; i++) {
						const a = state.pos[i].include.delete(letter);
						const b = state.pos[i].exclude.delete(letter);
						if (a || b) cleared = true;
					}
