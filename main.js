...
/**
 * Three-step lookahead E[steps] heuristic (deeper, slower).
 *
 * - First guess: g (fixed by caller).
 * - For each pattern bucket after g, choose best g2 (2nd guess).
 * - For each bucket after g2, choose best g3 (3rd guess),
 *   then use a leaf heuristic beyond g3.
 *
 * This is more accurate but significantly more expensive,
 * so it should only be used when |S| is already small.
 */
function EstepsOneLookaheadLocalDeep(S, g) {
  ...
  for (const [pat, arr] of parts.entries()) {
    ...
    if (pat === '22222') {
      est += p * 1; // 1 step (this guess)
      continue;
    }

    let bestG2Cost = Infinity;

    // Second guess g2 inside this bucket
    for (const g2 of arr) {
      const parts2 = partitionByPattern(arr, g2);
      let e2 = 0;
      ...
      for (const [pat2, arr2] of parts2.entries()) {
        ...
        if (bucket2Size === 1) {
          // Only one candidate left → 1 more step expected
          branchAfterG2 = 1;
        } else {
          // Do a full third-lookahead over g3 in arr2
          ...
          for (const g3 of arr2) {
            ...
            for (const [pat3, arr3] of parts3.entries()) {
              ...
              if (pat3 === '22222') {
                ...
                // No further lookahead, use leaf heuristic
                const leafCost = (bucket3Size === 1) ? 1 : 2;
                e3 += p3 * (1 + leafCost); // 1 for g3 + leaf
              }
            }
          ...
          branchAfterG2 = bestG3Cost;
        }

        // 1 for g2 + expected deeper cost
        e2 += p2 * (1 + branchAfterG2);
      }

      if (e2 < bestG2Cost) bestG2Cost = e2;
    }

    // 1 for g + best second-step structure
    est += p * (1 + bestG2Cost);
  }
...
async function suggestNext() {
	...
	try {
		...
		// Decide whether to use the deeper 3-step model
		const DEEP_ESTEP_THRESHOLD = 3200;  // tweak this if needed
		const useDeepEsteps = S.length <= DEEP_ESTEP_THRESHOLD;
