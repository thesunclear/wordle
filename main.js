...
/**
 * Hybrid one-/two-/three-lookahead E[steps] heuristic.
 *
 * - First guess: g1 (fixed by caller).
 * - For each pattern bucket after g1, we choose the best second guess g2.
 * - For each pattern bucket after g2:
 *     * If bucket is small enough, we do a third-lookahead over g3.
 *     * Otherwise we fall back to a simple leaf heuristic.
 *
 * This makes the estimate more accurate on small buckets,
 * and still keeps runtime manageable.
 *
 * Rough upper bound for a single branch:
 *   1 (g1) + 1 (g2) + 1 (g3) + 2 (leaf heuristic) = 5
 * so in this hybrid model we effectively allow E[steps] up to ≈ 5.
 */
function EstepsOneLookaheadLocal(S, g) {
  const n = S.length;
  if (n === 0) return 0;

  // When a bucket is this small or smaller, we are willing
  // to spend extra time doing a 3rd-step lookahead.
  const SMALL_CUTOFF = 7;
  ...
  for (const [pat, arr] of parts.entries()) {
    ...
    const p = bucketSize / n;

    // If g solves it immediately
    if (pat === '22222') {
      est += p * 1;  // cost = 1 (this guess g)
      continue;
    }

    // For this bucket, we look for the best g2 inside arr.
    let bestG2Cost = Infinity;
    ...
    for (const g2 of arr) {
      ...
      for (const [pat2, arr2] of parts2.entries()) {
        const bucket2Size = arr2.length;
        const p2 = bucket2Size / bucketSize;

        // If g2 solves it on this branch
        if (pat2 === '22222') {
          e2 += p2 * 1;   // cost = 1 (this guess g2)
          continue;
        }

        // --- Third-step logic (g3) / leaf heuristic ---
        let branchCostAfterG2;

        if (bucket2Size === 1) {
          // Only 1 candidate left: next guess will immediately solve it.
          // cost = 1(g2) + 1(next guess) → we only count the part AFTER g2 here
          branchCostAfterG2 = 1;  // "one more step expected"
        } else if (bucket2Size <= SMALL_CUTOFF) {
          // Small bucket: do a third lookahead over g3 in arr2
          let bestG3Cost = Infinity;
          ...
          for (const g3 of arr2) {
            ...
            for (const [pat3, arr3] of parts3.entries()) {
              const bucket3Size = arr3.length;
              const p3 = bucket3Size / bucket2Size;

              if (pat3 === '22222') {
                // g3 solves this branch immediately
                e3 += p3 * 1;  // cost = 1 (this guess g3)
              } else {
                // No further lookahead beyond g3.
                // Use a leaf heuristic for whatever remains.
                const leafCost = (bucket3Size === 1) ? 1 : 2;
                // 1 for taking g3, plus leafCost for the remaining guesses
                e3 += p3 * (1 + leafCost);
              }
            }

            if (e3 < bestG3Cost) bestG3Cost = e3;
          }

          // After g2, we expect "bestG3Cost" extra guesses in this branch.
          branchCostAfterG2 = bestG3Cost;
        } else {
          // Bucket is still large → fall back to simple heuristic,
          // similar to the original 2-step model.
          const leafCost = 2;  // "about two more guesses on average"
          branchCostAfterG2 = leafCost;
        }

        // Overall cost from this branch AFTER playing g2:
        // 1 for g2, plus expected deeper cost.
        e2 += p2 * (1 + branchCostAfterG2);
      }

      if (e2 < bestG2Cost) bestG2Cost = e2;
    }

    // Overall cost from this bucket AFTER playing g (g1):
    // 1 for g1, plus the best second-step structure.
    est += p * (1 + bestG2Cost);
  }
